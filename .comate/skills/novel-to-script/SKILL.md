...
配音选择贴合度规则：
- **性别必须匹配**：配音的性别必须与角色性别一致（男/女），不得跨性别选配音。
- **年龄段必须匹配**：配音的年龄段应尽量贴合角色设定（少年/青年/中年/老年），克隆音色库已按年龄段标注，选择时优先匹配。
- **声音特性必须匹配**：配音的嗓音特质（音色高低、明亮/低沉、轻柔/有力、气息感等）应与角色的性格、身份、说话气质贴合。例如豪迈武将选低沉有力的配音，斯文书生选柔和温润的配音，活泼少女选明亮轻快的配音。
- **语言必须匹配**：配音的语言必须为中文（或角色实际使用的语言），避免为中文小说角色选择英语、日语、西班牙语等外语配音。
- **主要角色配音不可重复**：同一部小说的主要角色（含旁白）必须使用不同的配音，不得出现两个主要角色共用同一配音的情况。普通配角和临时角色也尽量使用不同配音，只有戏份极少的龙套角色可在不同章节复用同一配音。
...
- **主要角色配音不可重复**：同一部小说的主要角色（含旁白）必须使用不同的配音（不同 `role_voice`），不得出现两个主要角色共用同一配音的情况。通过音色差异增强听众对角色区分的辨识度，避免听感混淆。
...
- **音色差异化检查**：同一部小说的主要角色（含旁白）必须使用不同的 `role_voice`，不得出现两个主要角色共用同一配音。普通配角也尽量不同，仅戏份极少的龙套可在不同章节复用。
- **性别匹配检查**：每个配音的性别（来自克隆音色库标注）必须与角色性别一致，不得跨性别选配音。
- **年龄段匹配检查**：配音的年龄段（少年/青年/中年/老年）应与角色设定尽量一致，显著性偏差（如老翁选了少年音）必须修正。
- **声音特性匹配检查**：配音的嗓音特质（音色高低、明亮或低沉、轻柔或有力）应与角色性格身份贴合，明显不匹配（如豪迈武将选了柔弱女声）必须修正。
- **语言匹配检查**：中文小说角色的配音语言必须为中文，不得使用英语、日语、西班牙语等外语配音。
